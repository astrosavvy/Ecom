...
function fallbackContent(route, seo) {
  ...
  if (route === '/') return `<main><h1>YOUNOYA — Coming Soon</h1><p>${escapeHtml(seo.description)}</p><p><a href="/shop">Preview 10 Consecrated Brooches</a> or <a href="/find-a-gift">Consult Gift Guide</a>.</p></main>`
  if (route === '/shop') return `<main><h1>The Younoya Consecrated Brooches</h1><p>${escapeHtml(seo.description)}</p><ul>${PRODUCTS.map(product => `<li><a href="/product/${product.handle}">${escapeHtml(product.name)}</a> — ${escapeHtml(product.price)}</li>`).join('')}</ul></main>`
