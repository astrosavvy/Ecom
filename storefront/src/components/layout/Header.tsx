...
export function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 w-full pointer-events-none">
      ...
      {/* Floating Header Bar (Left Seamless Logo + Center Navigation Pill + Right Direct Cart & Shop Buttons) */}
      <div className="pt-2 sm:pt-4 px-3 sm:px-8 max-w-7xl mx-auto flex items-center justify-between gap-3 sm:gap-4">
        ...
        <nav className="pointer-events-auto hidden md:flex items-center gap-1 px-5 py-2.5 rounded-full bg-[#FDFCF8]/95 backdrop-blur-2xl border border-[#E2E8E4] shadow-[0_8px_30px_rgb(0,0,0,0.06)] hover:shadow-[0_8px_35px_rgb(0,0,0,0.09)] transition-all duration-300">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="text-xs font-semibold px-3.5 py-1.5 rounded-full text-[#1C1C1C] hover:text-[#1C1C1C] hover:bg-[#E2E8E4]/70 transition-all whitespace-nowrap"
            >
              {item.label}
            </Link>
          ))}
        ...
        {/* 3. Desktop: Direct Standalone Cart & Shop Now Buttons (Outer Pill Removed) */}
        <div className="pointer-events-auto hidden md:flex items-center gap-3">
          <Link
            href="/cart"
            className="relative p-3 rounded-full bg-white/95 backdrop-blur-xl border border-[#E2E8E4] text-[#1C1C1C] hover:bg-[#F5F5F0] transition-all flex items-center justify-center shadow-md hover:scale-105 active:scale-95"
          ...
          <Link
            href="/products"
            className="px-6 py-3 rounded-full bg-[#1C1C1C] text-white text-xs font-extrabold uppercase tracking-wider hover:bg-[#333333] transition-all transform hover:scale-105 active:scale-95 shadow-md whitespace-nowrap"
          >
            Shop Now
          </Link>
        </div>

        {/* Mobile: Direct Standalone Cart & Menu Buttons (Outer Pill Removed) */}
        <div className="pointer-events-auto md:hidden flex items-center gap-2">
          <Link
            href="/cart"
            className="relative p-2.5 rounded-full bg-white/95 backdrop-blur-xl border border-[#E2E8E4] text-[#1C1C1C] flex items-center justify-center shadow-md"
            aria-label="View Cart"
          >
            <ShoppingBag size={16} />
            {totalItems > 0 && (
          ...
          <button
            ...
            className="p-2.5 rounded-full bg-white/95 backdrop-blur-xl border border-[#E2E8E4] text-[#1C1C1C] flex items-center justify-center shadow-md"
