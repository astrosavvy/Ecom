...
function mountLetsScroll(container, config) {
  ...
  function loadClip(s) {
    if (reduce || s.loading || !s.clip || !running) return;
    s.loading = true;
    const url = (isMobile() && s.clipM) ? s.clipM : s.clip;
    const ac = (typeof AbortController !== 'undefined') ? new AbortController() : null;
    if (ac) abortControllers.push(ac);
    fetch(url, ac ? { signal: ac.signal } : undefined)
      ...
      .then(blob => {
        ...
        s.objectUrl = objUrl;
        v.src = objUrl;
        v.addEventListener('loadedmetadata', () => { if (running) { s.ready = true; read(); } });
        v.addEventListener('seeked', () => { if (running) { s.el.classList.add('has-clip'); } }, { once: true });
  ...
  const onScroll = () => { if (!ticking && running) { ticking = true; requestAnimationFrame(read); } };
  ...
  // Intercept internal link clicks for SPA routers if onNavigate is configured
  const onContainerClick = (e) => {
    const a = e.target && e.target.closest ? e.target.closest('a') : null;
    if (!a) return;
    const href = a.getAttribute('href');
    if (href && href.startsWith('/') && !href.startsWith('//') && typeof config.onNavigate === 'function') {
      e.preventDefault();
  ...
  // Lifecycle destroy for clean unmount in SPA routers
  return function destroy() {
    ...
    const styleTag = document.getElementById('sw-css');
...
function injectCSS() {
  ...
  const css = `
  .sw-root{--sw-bg:#07080E;--sw-ink:#FFFBF0;--sw-ink-soft:#A39B8E;--sw-accent:#D4AF37;
    --sw-font-display:Outfit,"Instrument Serif",serif,sans-serif;
    --sw-font-body:Inter,system-ui,sans-serif;
    color:var(--sw-ink);font-family:var(--sw-font-body);}
  html,body{margin:0;background:var(--sw-bg,#07080E);overflow-x:hidden;}
  ...
  .sw-sky__glow{position:absolute;inset:0;background:radial-gradient(60% 42% at 74% 16%,color-mix(in srgb,var(--sw-accent) 22%,transparent),transparent 70%),radial-gradient(46% 34% at 50% 50%,color-mix(in srgb,#fff 10%,transparent),transparent 70%);}
  ...
  .sw-brand__name{font-family:var(--sw-font-display);font-weight:700;font-size:1.1rem;letter-spacing:0.08em;}
  .sw-nav{display:flex;gap:4px;padding:5px;background:rgba(26,26,30,0.6);backdrop-filter:blur(10px);border:1px solid color-mix(in srgb,var(--sw-accent) 26%,transparent);border-radius:999px;}
  ...
  .sw-nav__item:hover{color:var(--sw-ink);} .sw-nav__item.is-active{color:#1a1a1e;background:var(--sw-accent);font-weight:600;}
  .sw-topcta{text-decoration:none;font-weight:600;font-size:.9rem;color:#1a1a1e;background:var(--sw-accent);padding:10px 20px;border-radius:999px;white-space:nowrap;transition:transform .2s;}
  ...
  .sw-copy__num{font-family:ui-monospace,Menlo,monospace;font-size:.74rem;letter-spacing:.12em;color:var(--sw-accent);}
  ...
  .sw-copy__title{font-family:var(--sw-font-display);font-weight:700;color:var(--sw-ink);font-size:clamp(2rem,4.4vw,3.5rem);line-height:1.03;margin:12px 0 0;letter-spacing:-.01em;text-shadow:0 2px 20px rgba(0,0,0,0.7);}
  .sw-copy__body{margin-top:18px;font-size:clamp(1rem,1.25vw,1.14rem);line-height:1.55;color:var(--sw-ink-soft);max-width:40ch;text-shadow:0 1px 12px rgba(0,0,0,0.8);}
  .sw-copy__tags{list-style:none;display:flex;flex-wrap:wrap;gap:8px;margin:24px 0 0;padding:0;}
  .sw-copy__tags li{font-size:.82rem;font-weight:600;color:var(--sw-accent);padding:7px 14px;border-radius:999px;background:rgba(212,175,55,0.12);border:1px solid rgba(212,175,55,0.28);}
  ...
  .sw-btn--primary{color:#1a1a1e;background:var(--sw-accent);} .sw-btn--primary:hover{transform:translateY(-2px);}
  .sw-btn--ghost{color:var(--sw-ink);border:1.5px solid rgba(255,251,240,0.35);background:rgba(7,8,14,0.4);backdrop-filter:blur(8px);} .sw-btn--ghost:hover{transform:translateY(-2px);border-color:var(--sw-accent);}
  ...
  .sw-route__label{position:absolute;right:24px;top:50%;transform:translateY(-50%) translateX(6px);white-space:nowrap;font-size:.78rem;font-weight:600;color:var(--sw-ink);background:rgba(11,14,24,0.88);backdrop-filter:blur(6px);padding:5px 11px;border-radius:999px;opacity:0;pointer-events:none;transition:opacity .25s,transform .25s;border:1px solid rgba(212,175,55,0.3);}
  ...
  .sw-hint i{width:22px;height:34px;border-radius:12px;border:2px solid rgba(255,251,240,0.35);position:relative;}
